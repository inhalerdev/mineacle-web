...
function mineacle_lookup_name(PDO $pdo, string $historyTable, string $uuid, string $fallback = ''): string {
    ...
    if (!mineacle_table_exists($pdo, $historyTable)) {
        return $fallback !== '' ? $fallback : '#offline#';
    }
...
function fetch_litebans_bans_page(string $search = '', int $page = 1): array {
    ...
    if (!mineacle_table_exists($pdo, $bansTable)) {
        throw new RuntimeException('LiteBans table not found');
    }
    ...
    if ($search !== '') {
        ...
        if ($hasHistory) {
            $parts[] = "`uuid` IN (SELECT `uuid` FROM `" . mineacle_table($historyTable) . "` WHERE `name` LIKE :search)";
        }
    ...
    $sql = "SELECT " . implode(', ', $select) . " FROM `" . mineacle_table($bansTable) . "` $whereSql ORDER BY `time` DESC LIMIT :limit OFFSET :offset";
