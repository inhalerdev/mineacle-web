...
function mineacle_page_head(string $title): void {
    ...
    echo '<link rel="stylesheet" href="assets/bans-page.css?v=bansclean1.0.6">';
    echo '</head>';
    echo '<body>';
}
...
function mineacle_critical_styles(): void {
    echo <<<'HTML'
<style>
:root{--page-bg:#090314;--island:#0d0718;--rail:#10002f;--line:rgba(188,77,255,.22);--line-strong:rgba(218,121,255,.42);--text:#fff7ff;--muted:#b6a8c6;--button:#8d23ff;--shell-width:min(calc(100vw - 64px),1620px);--shell-left:max(32px,calc((100vw - var(--shell-width)) / 2));--rail-width:104px;--rail-gap:28px;--radius:8px}
...
.mineacle-bans-shell{width:var(--shell-width);min-height:calc(100vh - 378px);margin:0 auto;padding:46px 0 84px calc(var(--rail-width) + var(--rail-gap))}
.mineacle-bans-main{width:100%}
.mineacle-bans-rail{position:absolute;top:34px;left:var(--shell-left);bottom:40px;z-index:10;width:var(--rail-width);display:grid;grid-template-rows:auto 1fr auto;justify-items:center;gap:24px;padding:22px 0;border:1px solid var(--line-strong);border-radius:var(--radius);background:linear-gradient(180deg,rgba(25,4,66,.98),rgba(8,2,28,.98));box-shadow:0 28px 70px rgba(0,0,0,.46),inset 0 1px 0 rgba(255,255,255,.06)}
...
.mineacle-bans-rail-nav{display:flex;flex-direction:column;justify-content:center;gap:14px}.mineacle-bans-rail-link{width:56px;height:56px;display:grid;place-items:center;border:1px solid rgba(188,77,255,.2);border-radius:var(--radius);background:rgba(32,8,78,.76);text-decoration:none}.mineacle-bans-rail-link.is-active{border-color:rgba(245,178,255,.58);background:linear-gradient(135deg,#ff99f1,#9b25ff 54%,#4b00d8);box-shadow:0 0 0 5px rgba(157,37,255,.18),0 18px 34px rgba(147,26,235,.36)}.mineacle-bans-rail-link img{width:27px;height:27px;display:block;margin:auto;object-fit:contain;object-position:center;filter:brightness(0) invert(1)}.mineacle-bans-rail-link:first-child img{width:34px;height:34px;filter:none}.mineacle-bans-rail-support{margin-top:auto}
.mineacle-bans-search{width:min(100%,1120px);min-height:76px;display:grid;grid-template-columns:minmax(0,1fr) 64px;align-items:center;margin:0 auto;padding:0 10px 0 0;border:1px solid rgba(188,77,255,.2);border-radius:var(--radius);background:rgba(9,3,19,.96);box-shadow:0 22px 52px rgba(0,0,0,.36),inset 0 1px 0 rgba(255,255,255,.045);transition:border-color .14s ease,background .14s ease,box-shadow .14s ease}.mineacle-bans-search:focus-within,.mineacle-bans-search.has-value{border-color:rgba(218,121,255,.48);background:rgba(12,4,28,.98);box-shadow:0 24px 56px rgba(0,0,0,.4),0 0 0 5px rgba(157,37,255,.12),inset 0 1px 0 rgba(255,255,255,.06)}.mineacle-bans-search.is-typing{border-color:rgba(255,165,247,.6)}
...
.mineacle-footer-island{width:var(--shell-width);min-height:338px;display:grid;grid-template-columns:minmax(280px,1.35fr) repeat(3,minmax(180px,.72fr));gap:54px;align-items:start;margin:0 auto 40px;padding:52px 56px 30px;border:1px solid rgba(188,77,255,.2);border-radius:var(--radius);background:linear-gradient(180deg,rgba(18,5,39,.96),rgba(9,3,19,.98));box-shadow:0 28px 76px rgba(0,0,0,.4),inset 0 1px 0 rgba(255,255,255,.035)}
.mineacle-footer-brand-logo{width:min(100%,360px);display:inline-block;margin-bottom:22px}.mineacle-footer-brand-logo img{width:min(100%,340px);height:auto;display:block;object-fit:contain;filter:grayscale(1) brightness(1.16) contrast(1.05);opacity:.72;transition:filter .26s ease,opacity .26s ease,transform .26s ease}.mineacle-footer-brand-logo:hover img,.mineacle-footer-brand-logo:focus-visible img{filter:grayscale(0) brightness(1) contrast(1);opacity:1;transform:translateY(-1px)}.mineacle-footer-brand h2{margin:0 0 14px;color:#fff;font-size:31px;line-height:1.05;font-weight:950}.mineacle-footer-brand p{width:min(100%,360px);margin:0;color:var(--muted);font-size:16px;line-height:1.55;font-weight:780}
...
@media(max-width:1100px){:root{--shell-width:calc(100vw - 48px);--shell-left:24px;--rail-gap:20px}.mineacle-bans-rail{left:var(--shell-left)}.mineacle-footer-island{grid-template-columns:1.2fr 1fr 1fr;gap:36px 30px}.mineacle-footer-brand{grid-column:1/-1}}
@media(max-width:760px){:root{--shell-width:calc(100vw - 32px);--shell-left:16px}.mineacle-bans-shell{width:var(--shell-width);min-height:calc(100vh - 430px);padding:96px 0 40px}.mineacle-bans-rail{top:16px;left:var(--shell-left);right:var(--shell-left);bottom:auto;width:auto;height:64px;grid-template-columns:1fr;grid-template-rows:1fr;padding:8px}.mineacle-bans-rail-logo,.mineacle-bans-rail-support{display:none}.mineacle-bans-rail-nav{width:100%;flex-direction:row;justify-content:space-between;gap:8px}.mineacle-bans-rail-link{width:46px;height:46px}.mineacle-bans-rail-link img{width:22px;height:22px}.mineacle-bans-search{min-height:62px;grid-template-columns:minmax(0,1fr) 56px;padding-right:7px}.mineacle-bans-search-field,.mineacle-bans-search-field input{height:60px}.mineacle-bans-search-field{grid-template-columns:minmax(0,1fr)}.mineacle-bans-search-field input{padding-left:18px;font-size:15px}.mineacle-bans-search-action{width:44px;height:44px}.mineacle-bans-search-action:before{width:21px;height:21px}.mineacle-footer-island{min-height:0;grid-template-columns:1fr;gap:28px;padding:30px 22px 24px}.mineacle-footer-brand{grid-column:auto}.mineacle-footer-brand-logo{width:min(100%,280px)}.mineacle-footer-bottom{align-items:start;flex-direction:column;padding-top:8px}}
</style>
HTML;
}
...
function mineacle_footer(): void {
    ...
    echo '<img src="assets/m-studios.png?v=bansclean1.0.6" alt="Mineacle Studios">';
    echo '</a>';
    echo '<h2>Mineacle</h2>';
